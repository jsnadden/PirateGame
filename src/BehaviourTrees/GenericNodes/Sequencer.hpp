...
namespace BT
{
	...
	struct SequencerNode<BehaviourTree<Time, Args...>>
	...
	node_ptr<Tree> Sequencer(node_ptr<Tree> child, Children ... siblings)
	{
		std::vector<node_ptr<Tree>> children;
		children.push_back(std::move(child));
		(children.push_back(std::move(siblings)), ...);
		return std::make_unique<SequencerNode<Tree>>(std::move(children));
	}
